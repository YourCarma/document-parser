...
class CustomModelTranslator(AbstractTranslator):
    def __init__(self, source_text, source_language, target_language, include_image_in_output):
    ...
    async def translate(self, mode: ParserMods):
        ...
        for element, _level in doc.iterate_items():
            if isinstance(element, TextItem):
                ...
                text_item_translation_tasks.append(self.translate_element(element.text))
                text_elements.append(element)
            elif isinstance(element, TableItem):
                for cell in element.data.table_cells:
                    cell_item_transaltion_tasks.append(self.translate_element(cell.text))
                    cell_items.append(cell)
        

        if text_item_translation_tasks:
            logger.debug("Transalting text items")
        ...
        if cell_item_transaltion_tasks:
            logger.debug("Transalting cells in tables")
            transalted_cells = await asyncio.gather(*cell_item_transaltion_tasks)
            for cell, translated_text in zip(cell_items, transalted_cells):
        ...
        match mode:
            case ParserMods.TO_FILE:
                ...
                with NamedTemporaryFile(suffix=".md", delete=False) as tmp_file:
                    doc.save_as_markdown(filename=tmp_file.name,artifacts_dir=settings.ARTIFACTS_PATH, image_mode=self.image_mode)
