from modules.parser.v1.file_parsers.image_parser import ImageParser
from modules.parser.v1.file_parsers.docx_parser import DocParser
from modules.parser.v1.file_parsers.pptx_parser import PPTXParser
from modules.parser.v1.file_parsers.xlsx_parser import XLSXParser
from modules.parser.v1.file_parsers.docling_format_parser import (
    EMLParser,
    EPUBParser,
    ODPParser,
    ODSParser,
    ODTParser,
    TXTParser,
    XBRLParser,
)
from modules.parser.v1.file_parsers.pdf_parser import PDFParser
from modules.parser.v1.file_parsers.html_parser import HTMLParser
from modules.parser.v1.file_parsers.pdf_parser_vlm import PDFVLMParser
